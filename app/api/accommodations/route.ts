...
export async function GET(request: NextRequest) {
  try {
    ...
    const rows = all
      ? db.prepare("SELECT * FROM accommodations ORDER BY hotel_name, room_type").all()
      : db
          .prepare(
            "SELECT * FROM accommodations WHERE available = 1 ORDER BY hotel_name, price_per_night"
          )
          .all();

    return NextResponse.json({ success: true, accommodations: rows });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = db
      .prepare(
        `INSERT INTO accommodations (hotel_name, room_type, description, price_per_night, currency, max_guests, available)
         VALUES (?, ?, ?, ?, ?, ?, ?)`
      )
      .run(
        hotel_name.trim(),
    ...
    const created = db
      .prepare("SELECT * FROM accommodations WHERE id = ?")
      .get(result.lastInsertRowid);

    return NextResponse.json({ success: true, accommodation: created }, { status: 201 });
  } catch (error) {
